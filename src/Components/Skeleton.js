  ...
  return (
    <div className="flex flex-col items-center justify-evenly min-[520px]:flex-row flex-wrap">
      {Array.from(Array(20)).map((e, index) => (
        <div
          className="w-full basis-1/4 min-w-[12rem] lg:px-2 py-2 animate-pulse"
          key={index}
        >
  ...
  return (
    <div className="flex flex-col overflow-hidden sm:basis-4/5 gap-3 mt-4 mx-4 lg:mx-auto">
      {Array.from(Array(10)).map((e, index) => (
        <div
          className="w-full flex gap-2 pr-4 justify-center animate-pulse"
          key={index}
        >
...
const Skeleton = ({ type }) => {
  ...
  return (
    <div className="text-gray-500 font-bold my-4 mx-auto animate-bounce">
      Loading...
    </div>
  );
};
